...
public class ReservasiView extends JPanel {
    ...
    private void updateStatus(String status) {
        ...
        try {
            ...
            // Gunakan method checkIn() dan checkOut() untuk update otomatis status kamar
            boolean success = false;
            if ("Check-In".equals(status)) {
                success = ctrl.checkIn(idReservasi);
            } else if ("Check-Out".equals(status)) {
                success = ctrl.checkOut(idReservasi);
            }
            
            if (success) {
                JOptionPane.showMessageDialog(this, "Status diubah menjadi " + status + ". Status kamar otomatis terupdate!");
                loadTable();
                clearForm();
            } else {
                JOptionPane.showMessageDialog(this, "Gagal mengubah status. Cek data dan koneksi.");
            }
        } catch (NumberFormatException ex) {
